...
class OpenAIService {
    static async analyzeProductImage(imageBase64, mimeType = 'image/jpeg') {
...
✅ "title": "Coca-Cola 350ml"`;
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new Error('TIMEOUT - OpenAI demorou mais que 8s')), 8000);
            });
            ...
            if (content.includes('Nome do produto') ||
    ...
    static async analyzeMultipleProductImages(imagesBase64, mimeTypes = []) {
...
- Preços realistas baseados no tipo de produto`;
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new Error('TIMEOUT - OpenAI demorou mais que 30s')), 30000);
            });
            const completion = (await Promise.race([
                openai.chat.completions.create({
                    model: 'gpt-4o-mini',
            ...
            console.log('📏 Tamanho da resposta:', responseContent.length, 'caracteres');
            console.log('🎯 Primeiros 200 caracteres:', responseContent.substring(0, 200));
            console.log('🎯 Últimos 200 caracteres:', responseContent.substring(responseContent.length - 200));
            // Verificar se a resposta foi cortada
            if (responseContent.length >= 490) {
                console.warn('⚠️ [DEBUG] Resposta pode ter sido cortada pelo limite de tokens!');
            }
            // Tentar extrair JSON da resposta
            ...
            if (!jsonMatch) {
                ...
                throw new Error('RESPOSTA_SEM_JSON - OpenAI não retornou formato JSON válido');
            }
            console.log('🔍 JSON extraído:', jsonMatch[0]);
            let productData;
            try {
                ...
                throw new Error('JSON_PARSE_ERROR - JSON retornado pela OpenAI é inválido');
            }
            // Validar se os campos obrigatórios estão presentes
            ...
            if (missingFields.length > 0) {
                ...
                console.error('🔍 Dados completos recebidos:', JSON.stringify(productData, null, 2));
                throw new Error(`CAMPOS_OBRIGATORIOS_AUSENTES - Campos ausentes: ${missingFields.join(', ')}`);
            }
...
`;
     ...
     */
    static async processMenuOCR(imageBase64, mimeType = 'image/jpeg') {
...
AGORA ANALISE ESTA IMAGEM e extraia TODOS OS PRODUTOS:`;
            try {
                ...
                const jsonMatch = content.match(/\{[\s\S]*\}/);
                if (!jsonMatch) {
                    console.error('❌ [DEBUG] JSON não encontrado na resposta:', content.substring(0, 300));
                ...
                console.log('🔍 [DEBUG CARDÁPIO] Estrutura parseada:', JSON.stringify(parsedResult, null, 2));
                if (!parsedResult.products || !Array.isArray(parsedResult.products)) {
                    console.error('❌ [DEBUG CARDÁPIO] Estrutura inválida - esperado array "products":', parsedResult);
                    throw new Error('Formato de resposta inválido - products não encontrado');
                }
                console.log(`🔍 [DEBUG CARDÁPIO] Produtos no JSON: ${parsedResult.products.length}`);
                ...
                if (parsedResult.products.length === 1) {
                    ...
                    console.error('🔍 [ANÁLISE] Produto extraído:', parsedResult.products[0]);
                    console.error('🔍 [DICA] Verifique se a imagem realmente contém vários produtos');
                    console.error('🔍 [DICA] Se houver vários produtos, o prompt precisa ser melhorado');
                    ...
                    throw new Error('OpenAI Vision retornou apenas 1 produto - forçando fallback para Tesseract');
                }
                else if (parsedResult.products.length > 1) {
                    console.log(`✅ [SUCESSO] ${parsedResult.products.length} produtos extraídos corretamente!`);
                    console.log('🔍 [PRODUTOS] Lista de títulos:', parsedResult.products.map((p) => p.title).join(', '));
                }
                ...
                for (const product of parsedResult.products) {
                    if (product.title && product.price) {
                        ...
                        if (isNaN(price) || price <= 0) {
                            console.warn(`⚠️ Preço inválido: ${product.price}, usando preço padrão`);
                        ...
                        const validProduct = {
                            ...
                            description: product.description || `Delicioso ${title.toLowerCase()}`,
                            price: price,
                            offer: product.offer && !isNaN(parseFloat(product.offer))
                                ? parseFloat(product.offer)
                                : Math.round(price * 0.9 * 100) / 100, // 10% desconto arredondado
                        };
                        console.log(`✅ Produto validado: "${validProduct.title}" - R$ ${validProduct.price}`);
                ...
                console.log(`✅ OpenAI Vision: ${validProducts.length} produtos extraídos em ${totalTime}ms`);
                return {
                    success: true,
     ...
     */
    static async processMenuWithTesseract(imageBase64, originalStartTime) {
        ...
        try {
            ...
            const extractedText = await node_tesseract_ocr_1.default.recognize(imageBuffer, {
            ...
            console.log(`✅ Tesseract: ${products.length} produtos extraídos em ${totalTime}ms`);
            return {
                success: true,
                extractedText,
                products,
     ...
     */
    static async parseMenuText(text) {
...
AGORA ANALISE O TEXTO E EXTRAIA TODOS OS PRODUTOS:`;
        try {
            ...
            console.log(`✅ ${validProducts.length} produtos identificados na análise de texto`);
