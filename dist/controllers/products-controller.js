...
class ProductsController {
    ...
    compressBase64Image(base64, maxSizeKB = 500) {
        ...
        if (base64.length > maxLength) {
            console.warn(`⚠️ Imagem muito grande (${currentSizeKB}KB), truncando para ${maxSizeKB}KB`);
    ...
    async generateWithAI(request, reply) {
        ...
        try {
            ...
            console.log(`📊 Tamanho base64: ${Math.round((imageBase64.length * 3) / 4 / 1024)} KB`);
            ...
            try {
                ...
                generatedProduct = await openai_service_1.OpenAIService.analyzeProductImage(imageBase64);
    ...
    async generateWithMultipleAI(request, reply) {
        ...
        try {
            ...
            if (files.length === 0) {
                console.error('❌ Nenhum arquivo enviado');
                throw new AppError_1.AppError('Pelo menos uma imagem é obrigatória para análise da IA', 400);
            }
            if (files.length > 5) {
                throw new AppError_1.AppError('Máximo de 5 imagens permitidas', 400);
            }
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                console.log(`📊 Imagem ${i + 1} - Base64: ${Math.round((imageBase64.length * 3) / 4 / 1024)} KB`);
            }
            ...
            try {
                ...
                console.log('🔍 Tentando análise com OpenAI Vision (múltiplas imagens)...');
                generatedProduct = await openai_service_1.OpenAIService.analyzeMultipleProductImages(imagesBase64);
                ...
                if (aiError.message === 'RATE_LIMIT' ||
                    ...
                    try {
                        generatedProduct = await openai_service_1.OpenAIService.analyzeProductImage(imagesBase64[0]);
